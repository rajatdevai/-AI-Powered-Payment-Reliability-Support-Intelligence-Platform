...
from src.agent_service import AgentServiceServicer
from src.config import settings

# Setup logging
logging.basicConfig(level=settings.LOG_LEVEL, format="%(asctime)s [%(levelname)s] %(name)s: %(message)s")
logger = logging.getLogger("agent-service.main")

# FastAPI setup
app = FastAPI(title="PRISM Agent OS HTTP Gateway", version="1.0.0")
...
async def run_grpc_server():
    ...
    agent_pb2_grpc.add_AgentServiceServicer_to_server(
        AgentServiceServicer(), grpc_server
    )
