...
- **Rules**:
  - Follow PEP 8 (Python) / ESLint + Prettier (TypeScript). Add type hints. Use structured logging.
...
## 4. PRISM-Specific Constraints

These rules apply in addition to the standard phases above and are specific to this repository.

### 4.1 Architecture Boundaries (Never Violate)
- **No REST between microservices.** Node ↔ Python sync calls use **gRPC only**. Async calls use **Kafka only**.
- **No business logic in the Supervisor Agent.** It routes. Sub-agents execute.
- **No direct DB access from frontend.** All data flows through api-gateway.
- **Redis IRIS is agent memory only.** It is not a general application cache.
- **Kafka topics are append-only.** Never delete or mutate a published event.

### 4.2 Language Allocation (Never Cross Without Justification)
| Workload | Language | Reason |
|---|---|---|
| API routing, real-time, WebSocket, I/O | Node.js + TypeScript + Fastify | Concurrency, event-driven |
| ML scoring, graph traversal, LLM, RAG | Python + FastAPI | Ecosystem maturity |
| Frontend (all three apps) | Next.js 15 + TypeScript | Enterprise UI, SSR, streaming |

### 4.3 Database Write Rules
| Database | Who Writes | Who Reads |
|---|---|---|
| PostgreSQL | payment-orchestrator, rca-engine, reversal-engine, incident-engine | api-gateway, agent-service |
| Redis (hot cache) | route-health-engine, bank-health-engine, prediction-engine | payment-orchestrator, prediction-engine, frontend (via api-gateway) |
| Redis IRIS | agent-service only | agent-service only |
| Neo4j | blast-radius-engine (writes blast radius), infra seed | rca-engine, blast-radius-engine |
| Qdrant | infra seed, rca-engine (stores generated RCAs) | rca-engine, agent-service (knowledge-agent) |

### 4.4 Kafka Topic Ownership
| Topic | Publisher | Consumers |
|---|---|---|
| `payment.initiated` | payment-orchestrator | incident-engine, websocket-gateway |
| `payment.success` | payment-orchestrator | websocket-gateway, notification-service |
| `payment.failed` | payment-orchestrator | rca-engine, reversal-engine, blast-radius-engine, incident-engine, websocket-gateway |
| `payment.timeout` | payment-orchestrator | incident-engine, rca-engine |
| `route.metrics` | route-health-engine | incident-engine |
| `incident.created` | incident-engine | blast-radius-engine, websocket-gateway, notification-service |
| `incident.resolved` | incident-engine | websocket-gateway, notification-service |
| `rca.generated` | rca-engine | payment-orchestrator (updates Postgres), websocket-gateway |
| `reversal.predicted` | reversal-engine | payment-orchestrator (updates Postgres), websocket-gateway |
| `blast_radius.calculated` | blast-radius-engine | websocket-gateway |
| `fraud.detected` | fraud-engine | incident-engine, notification-service |
| `support.case.created` | api-gateway | notification-service |
| `support.case.closed` | api-gateway | notification-service |

---

*This file is the law of this repository. Every prompt, every phase, every line of code must be consistent with it.*
